...
def generate_all_responses() :
    ...
    struct = 'lwr32'
    fun = lambda x: 0*x+1
    resp = generate_responses(isos, fun, struct=struct, name='lwr32_flat_resp.p', overwrite=True)
    pwr = Flux(7.0, 600.0)
    resp = generate_responses(isos, pwr.evaluate, struct=struct, name='lwr32_pwr_resp.p', overwrite=True)
    triga = Flux(2.0, 600.0) 
    resp = generate_responses(isos, triga.evaluate, struct=struct, name='lwr32_triga_resp.p', overwrite=True)
    ...
    struct = 'scale252'
    fun = lambda x: 0*x+1
    resp = generate_responses(isos, fun, struct=struct, name='scale252_flat_resp.p', overwrite=True)
    pwr = Flux(7.0, 600.0)
    resp = generate_responses(isos, pwr.evaluate, struct=struct, name='scale252_pwr_resp.p', overwrite=True)
    triga = Flux(2.0, 600.0) 
    resp = generate_responses(isos, triga.evaluate, struct=struct, name='scale252_triga_resp.p', overwrite=True)
...
if __name__ == "__main__" :
    ...
    resp1 = generate_responses(isos, None, name='scale252_flat_resp.p', overwrite=False)
    resp2 = generate_responses(isos, None, name='scale252_pwr_resp.p', overwrite=False)

    for iso in isos:
        x, y = plot_multigroup_data(resp1['eb'],  resp2[iso])
        plt.loglog(x, y, ls='-',color=isos_colors[iso])
        plt.legend(isos_str.values())